...
public class RelationParser {
	...
	public static void main(String args[]) {
		...
		RelationParser a = new RelationParser(gitRepository, "camel",
				"D:\\Workspace\\co-change summer\\understand\\camel\\");
		a.extractRelation();
	}
	...
	private void parseFile(File f) {
		...
		for (int i = 0; i < fileList.size(); i++)
			for (int j = i + 1; j < fileList.size(); j++) {
				FilePairCount dataList = FilePairCountDAO.selectByRepositoryIdAndFilePairName(
						this.repository.getRepositoryId(), fileList.get(i) + "||" + fileList.get(j));
				if (dataList == null) {
